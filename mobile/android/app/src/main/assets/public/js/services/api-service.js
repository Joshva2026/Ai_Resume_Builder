...
const ApiService = (() => {
  // API URL configuration: route through same-origin /api endpoint on web,
  // or the configured production backend URL on mobile/Capacitor.
  let BASE_URL = '/api';
  if (
    ...
    BASE_URL = 'https://ai-resume-builder-rb1m.onrender.com/api';
  }
      ...
      return new Promise((resolve, reject) => {
        ...
        const pollTimer = setInterval(() => {
          if (popup.closed) {
            clearInterval(pollTimer);
            window.removeEventListener('message', messageHandler);
            // Check if tokens were set in localStorage by popup
            const token = getToken();
            if (token) {
